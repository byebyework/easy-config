...
from .models.nested_config import NestedConfig
...
def auto_loader(config_path=None, env=None):
    ...
    if env is None:
        ...
        env = os.environ.get('ENV', os.environ.get(
            'ENVIRONMENT', os.environ.get('PROFILE', 'dev')))
    ...
    if config_path is None:
        ...
        if env:
            common_config_files.extend([
                os.path.join(
                    root_dir, f"config-{env}.yaml"), os.path.join(root_dir, f"config-{env}.yml"),
                os.path.join(root_dir, f"config-{env}.json"),
                os.path.join(
                    root_dir, f"config-{env}.ini"), os.path.join(root_dir, f"config-{env}.cfg"),
                os.path.join(
                    root_dir, f"application-{env}.yaml"), os.path.join(root_dir, f"application-{env}.yml"),
                os.path.join(root_dir, f"application-{env}.json"),
                os.path.join(root_dir, f"application-{env}.properties"), os.path.join(
                    root_dir, f"application-{env}.ini"),
            ...
            if os.path.exists(config_dir) and os.path.isdir(config_dir):
                common_config_files.extend([
                    os.path.join(
                        config_dir, f"config-{env}.yaml"), os.path.join(config_dir, f"config-{env}.yml"),
                    os.path.join(config_dir, f"config-{env}.json"),
                    os.path.join(
                        config_dir, f"config-{env}.ini"), os.path.join(config_dir, f"config-{env}.cfg"),
                    os.path.join(config_dir, f"application-{env}.yaml"), os.path.join(
                        config_dir, f"application-{env}.yml"),
                    os.path.join(config_dir, f"application-{env}.json"),
                    os.path.join(config_dir, f"application-{env}.properties"), os.path.join(
                        config_dir, f"application-{env}.ini"),
        ...
        common_config_files.extend([
            ...
            os.path.join(root_dir, "config.yaml"), os.path.join(
                root_dir, "config.yml"),
            os.path.join(root_dir, "config.json"),
            os.path.join(root_dir, "config.ini"), os.path.join(
                root_dir, "config.cfg"),
            os.path.join(root_dir, "application.yaml"), os.path.join(
                root_dir, "application.yml"),
            os.path.join(root_dir, "application.json"),
            os.path.join(root_dir, "application.properties"), os.path.join(
                root_dir, "application.ini")
        ])
        ...
        if os.path.exists(config_dir) and os.path.isdir(config_dir):
            common_config_files.extend([
                ...
                os.path.join(config_dir, "config.yaml"), os.path.join(
                    config_dir, "config.yml"),
                os.path.join(config_dir, "config.json"),
                os.path.join(config_dir, "config.ini"), os.path.join(
                    config_dir, "config.cfg"),
                os.path.join(config_dir, "application.yaml"), os.path.join(
                    config_dir, "application.yml"),
                os.path.join(config_dir, "application.json"),
                os.path.join(config_dir, "application.properties"), os.path.join(
        ...
        if config_path is None:
            logger.warning(
                "No configuration file found, returning empty configuration")
    ...
    if env and config_path:
        ...
        if os.path.exists(env_config_path):
            logger.info(
                f"Loading environment-specific configuration file: {env_config_path}")
...
def _load_config_file(config_path):
    ...
    if file_name == '.env' or file_name.startswith('.env.'):
        ...
        logger.info(
            f"Unrecognized file type: {file_name}, trying to infer from content")
            ...
            for key, value in env_dict.items():
                if isinstance(value, dict):
                    ...
                    if base_value and hasattr(base_value, 'as_dict'):
                        ...
                        merged_obj = _merge_nested_dict(base_value, value, NestedConfig)

                        setattr(self, key, merged_obj)
                    else:
                        # Otherwise create a new nested object
                        nested_obj = NestedConfig()

                        for k, v in value.items():
                            if isinstance(v, dict):
                                # Handle deeper nesting
                                sub_obj = NestedConfig()
                                for sk, sv in v.items():
                                    setattr(sub_obj, sk, sv)
                                setattr(nested_obj, k, sub_obj)
                            else:
            ...
            if '.' in key:
                ...
                for part in parts[:-1]:
                    try:
                        ...
                        # If not found in the environment configuration, check the base configuration
                        return key in self._base if hasattr(self._base, '__contains__') else False
                return hasattr(obj, parts[-1]) or (hasattr(self._base, '__contains__') and key in self._base)
            return hasattr(self, key) or (hasattr(self._base, '__contains__') and key in self._base)

    return MergedConfig(base_config, env_dict)


def _merge_nested_dict(base_obj, env_dict, nested_class=None):
    """
    ...
    Args:
        ...
        nested_class (class, optional): Class to use for nested objects. If None, uses a simple dynamic class.
    ...
    if hasattr(base_obj, 'as_dict'):
        ...
        base_dict = {k: v for k, v in base_obj.__dict__.items()
    ...
    for key, value in env_dict.items():
        if isinstance(value, dict):
            ...
            if base_value and (hasattr(base_value, 'as_dict') or hasattr(base_value, '__dict__')):
                ...
                merged = _merge_nested_dict(base_value, value, nested_class)
                setattr(nested_obj, key, merged)
            else:
                # Otherwise create a new nested object
                if nested_class:
                    sub_obj = nested_class()
                else:
                ...
                for k, v in value.items():
                    if isinstance(v, dict):
                        # Handle deeper nesting
                        if nested_class:
                            deeper_obj = nested_class()
                        else:
                            deeper_obj = type('NestedConfig', (), {
                                '__getitem__': lambda self, key: getattr(self, key),
                                '__contains__': lambda self, key: hasattr(self, key)
                            })()
                        for dk, dv in v.items():
                            setattr(deeper_obj, dk, dv)
                        setattr(sub_obj, k, deeper_obj)
                    else:
                        setattr(sub_obj, k, v)
...
def _load_by_content(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if not lines:
                logger.warning(
                    f"File content is empty or contains only comments: {file_path}")
                return type('EmptyConfig', (), {
                    'get': lambda self, key, default=None: default,
                    'as_dict': lambda self: {},
                    '__repr__': lambda self: "EmptyConfig()",
            ...
            if env_line_count > 0 and env_line_count / len(lines) >= 0.5:
                logger.info(
                    f"Determined to be ENV format based on content: {file_path}")
            ...
            if content.startswith('{') and content.endswith('}'):
                try:
                    json.loads(content)
                    logger.info(
                        f"Determined to be JSON format based on content: {file_path}")
            ...
            if any(line.startswith('[') and line.endswith(']') for line in lines):
                try:
                    ...
                    if len(config.sections()) > 0:
                        logger.info(
                            f"Determined to be INI format based on content: {file_path}")
            ...
            has_indentation = any(line.startswith(
            ...
            if (yaml_indicators or has_indentation) and not any(line.startswith('[') and line.endswith(']') for line in lines):
                try:
                    ...
                    if yaml_data is not None:  # Ensure successful parsing
                        logger.info(
                            f"Determined to be YAML format based on content: {file_path}")
            ...
            if any('=' in line for line in lines):
                logger.info(
                    f"File type not clearly identified, but contains equals signs, trying as ENV format: {file_path}")
                return env_loader(file_path)
            ...
            try:
                ...
                if yaml_data is not None:
                    logger.info(
                        f"File type not clearly identified, trying as YAML format: {file_path}")
                    return yaml_loader(file_path)
            except yaml.YAMLError:
                pass
            ...
            logger.warning(
                f"Could not identify file type, defaulting to env_loader: {file_path}")
            return env_loader(file_path)
